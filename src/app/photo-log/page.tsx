...
export default function PhotoLogPage() {
  ...
  async function handleAddItem(item: AiFoodEstimateItem) {
    await addEntry(slot, {
  ...
  if (!user) {
    return <p className="text-center text-slate-400">Sign in to use the AI photo estimator.</p>;
  }

  return (
    <div className="flex flex-col gap-6">
      <header>
        <h1 className="text-xl font-bold">AI Photo Log</h1>
            ...
            {estimate.items.map((item, i) => (
              <li key={i} className="flex items-center justify-between px-4 py-3 text-sm">
                ...
                <button
                  ...
                  className="rounded-lg bg-brand-50 px-3 py-1.5 text-xs font-medium text-brand-600 hover:bg-brand-100"
                >
                  Add
                </button>
              </li>
